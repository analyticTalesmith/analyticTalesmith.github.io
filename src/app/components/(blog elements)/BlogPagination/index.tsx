...
function getPaginationRange( radius: number, currentPage: number, totalPages : number ): number[] {
    let startPage = Math.max(1, currentPage - radius);
    let endPage = Math.min(currentPage + radius, totalPages);

    let startShift = Math.max(0, radius - (currentPage - startPage));
    let endShift = Math.max(0, radius - (totalPages - currentPage));

    const pageNumbers = []

    for (let i = startPage - endShift; i <= endPage + startShift; i++) {
        pageNumbers.push(i);
    }
...
const BlogPagination = ({ currentPage, totalPages }: PaginationProps) => {
    const postRootURL = '/params';
    const pageParam = 'page=';
    const pageShowRadius = 2;
    const pageNumbers = getPaginationRange(pageShowRadius, currentPage, totalPages);
    ...
    return (
        <nav className="flex row my-auto w-auto">
            <ul className="flex mx-auto overflow-hidden border-2 border-black rounded-lg">
                ...
                {currentPage + pageShowRadius < totalPages && (
